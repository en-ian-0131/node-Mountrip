...
const getdifficultyDataHard = async (req, res) => {
  ...
  // )
  Rows = await db.query(
    'SELECT rating.score,difficulty_list.difficulty_short,trails.sid,trails.trail_img,trails.trail_name,trails.geo_location_sid,trails.geo_location_town_sid,trails.price FROM `trails` INNER JOIN difficulty_list ON trails.difficulty_list_sid=difficulty_list.sid INNER JOIN rating ON rating.trails_sid=trails.sid WHERE difficulty_list_sid=3 LIMIT 6'
  )
  const rows = Rows[0]
  // res.json(rows)
  // console.log({ rows })
...
//   //   const rows = await db.query(sql,[req.body.lastSid,req.body.cartSid,req.body.cartQuantity])
//   //   res.json(rows)
    for(let i=0 ; i<req.body.cartSid.length ; i++){
      for(j=0; j<req.body.cartQuantity.length ; j++){
          if(i===j){
           const sql ='INSERT INTO `order_detail`(`order_sid`, `batch_sid`,`amount`,`create_date`,`fake_delete`) VALUES (?,?,?,now(),0)'
           const [rows] = await db.query(sql,[req.body.lastSid,req.body.cartSid[i],req.body.cartQuantity[j]])
...
router.post('/orderDates', async (req, res) => {
  const sql =
...
// router.post('/history3', async (req, res) => {
//   console.log('req.body:', req.body)
//   const sql =
//     "INSERT INTO `order_list`( `order_date`, `member_sid`,`order_status_sid` ,`total`,`memo`,`fake_delete`) VALUES (now(),?,2,?,null,'1')"
//   const rows = await db.query(sql, [req.body.userSid, req.body.newPayTotal])
...
//         if(i===j){
//          const sql ='INSERT INTO `order_detail`(`order_sid`, `batch_sid`,`amount`,`create_date`,`fake_delete`) VALUES (?,?,?,now(),0)'
//          const rows = await db.query(sql,[orderList_lastSid,req.body.cartSid[i],req.body.cartQuantity[j]])
//          const Rows = rows[0]
//          console.log(Rows)
//          res.json(Rows)
//         }
